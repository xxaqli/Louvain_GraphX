...
object Louvain {
  ...
  def execute(sc:SparkContext, initG:Graph[None.type,Double]) : RDD[(Long, List[Long])] ={
    ...
    var community = CommUtil.getCommunities(louvainG)
    while (res._2 != 0 && curIter < 20) {
      louvainG = res._1
      louvainG = step2(louvainG)
      community = CommUtil.getCommunities(louvainG)
  ...
  def existSuccessFile(sc:SparkContext, pathString :String): Boolean ={
    val path = new Path(pathString)
    val fs = path.getFileSystem(sc.hadoopConfiguration)
    val isExists = fs.exists(new Path(pathString))
    isExists
  }

  def deleteHadoopFile(sc: SparkContext, pathString: String): Unit ={
    val fs = new Path(pathString).getFileSystem(sc.hadoopConfiguration)
  ...
  def main(args: Array[String]) {
    ...
    val sourcePath = "/Users/binli/Desktop/unionpay/GraphX/Project/louvain_community_detection/data"

    val sc = new SparkContext(conf)
    val initG = GraphUtil.loadInitGraph(sc, sourcePath+"/club.txt")
    val community = execute(sc, initG)

    if(existSuccessFile(sc, sourcePath+"/community"))
      deleteHadoopFile(sc, sourcePath+"/community")

    community.repartition(1).map{case (cid, vetices) => s"$cid|-${vetices.mkString(" ")}"}.saveAsTextFile(sourcePath+"/community")
    sc.stop()
  }
